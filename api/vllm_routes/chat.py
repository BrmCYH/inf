...
async def create_chat_completion(
    ...
    if not query_text.startswith('Use the following context as your learned knowledge, inside <context></context>') and '你是凤凰' in request.messages[0]['content']:
        logger.info('add RAG')
        request.messages[-1]['content'], tag_link_paths = phoenix_kg_wrapper.wrap_question(query_text)
